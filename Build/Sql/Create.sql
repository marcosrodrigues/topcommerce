...
ALTER TABLE dbo.PEDIDOS_VENDA ADD
	FUNCIONARIO_CODIGO char(6) NULL
GO
ALTER TABLE dbo.PEDIDOS_VENDA ADD CONSTRAINT
	FK_PEDIDOS_VENDA_FUNCIONARIOS FOREIGN KEY
	(
	FUNCIONARIO_CODIGO
...
CREATE TABLE [dbo].[BAIXAS_CONTA_RECEBER](
	[ID] [int] IDENTITY(1,1) NOT NULL,
	[CONTA_RECEBER_ID] [int] NOT NULL,
	[DATA] [datetime] NOT NULL,
	[VALOR] [float] NOT NULL,
	[FORMA_PAGAMENTO] [int] NOT NULL,
 CONSTRAINT [PK_BAIXAS_CONTA_RECEBER] PRIMARY KEY CLUSTERED 
(
	[ID] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
...
ALTER TABLE [dbo].[BAIXAS_CONTA_RECEBER]  WITH CHECK ADD  CONSTRAINT [FK_BAIXAS_CONTA_RECEBER_CONTAS_RECEBER] FOREIGN KEY([CONTA_RECEBER_ID])
REFERENCES [dbo].[CONTAS_RECEBER] ([ID])
GO

ALTER TABLE [dbo].[BAIXAS_CONTA_RECEBER] CHECK CONSTRAINT [FK_BAIXAS_CONTA_RECEBER_CONTAS_RECEBER]
GO

ALTER TABLE dbo.CONTAS_RECEBER ADD
	RESTANTE float(53) NULL
GO

update contas_receber set restante = valor
