object FramePesquisaTipoProduto: TFramePesquisaTipoProduto
  Left = 0
  Top = 0
  Width = 561
  Height = 25
  AutoSize = True
  TabOrder = 0
  object Label3: TLabel
    Left = 0
    Top = 4
    Width = 76
    Height = 13
    Caption = 'Tipo de Produto'
  end
  object edtCodigoTipoProduto: TEdit
    Left = 96
    Top = 1
    Width = 35
    Height = 21
    MaxLength = 3
    TabOrder = 0
    OnExit = edtCodigoTipoProdutoExit
    OnKeyDown = edtCodigoTipoProdutoKeyDown
  end
  object bbtConsultarTipoProduto: TBitBtn
    Left = 131
    Top = 0
    Width = 30
    Height = 25
    DoubleBuffered = True
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentDoubleBuffered = False
    TabOrder = 1
    OnClick = bbtConsultarTipoProdutoClick
  end
  object edtDescricaoTipoProduto: TEdit
    Left = 161
    Top = 1
    Width = 400
    Height = 21
    Enabled = False
    TabOrder = 2
  end
end
